<script lang="ts">
  ...
  async function handleImport() {
    ...
    importError = "";
    try {
      const result = await ImportSMIMECertificateFromPath(
        accountId,
        importFilePath,
        importPassword
      );
      addToast({
        type: "success",
        message: $_("security.certImported", {
          values: { count: result.chainLength }
        })
      });
  ...
  async function handleImportBER() {
    ...
    importError = "";
    try {
      const result = await ImportSMIMECertificateFromPathBER(
        accountId,
        importFilePath,
        importPassword
      );
  ...
  async function handleSetDefault(certId: string) {
    try {
      ...
      addToast({
        type: "error",
        message: $_("security.failedToSetDefaultCert")
      });
    }
  }
  ...
  async function handleSignPolicyChange(policy: string) {
    try {
      ...
      addToast({
        type: "error",
        message: $_("security.failedToUpdateSignPolicy")
      });
    }
  }
  ...
  async function handleEncryptPolicyChange(policy: string) {
    try {
      ...
      addToast({
        type: "error",
        message: $_("security.failedToUpdateEncryptPolicy")
      });
    }
  }
  ...
  async function handleImportRecipientCert() {
    ...
    recipientImportError = "";
    try {
      await ImportRecipientCert(
        recipientImportEmail.trim(),
        recipientImportFilePath
      );
      addToast({
        type: "success",
        message: $_("security.recipientCertImported", {
          values: { email: recipientImportEmail.trim() }
        })
      });
  ...
  async function handleDeleteSenderCert(certId: string) {
    try {
      ...
      addToast({
        type: "error",
        message: $_("security.failedToRemoveSenderCert")
      });
    }
  }
  ...
  async function handleSetDefaultPGP(keyId: string) {
    try {
      ...
      addToast({
        type: "success",
        message: $_("security.defaultPGPKeyUpdated")
      });
      await loadData();
    } catch (err) {
      ...
      addToast({
        type: "error",
        message: $_("security.failedToSetDefaultPGPKey")
      });
    }
  }
  ...
  async function handlePGPSignPolicyChange(policy: string) {
    try {
      ...
      addToast({
        type: "error",
        message: $_("security.failedToUpdatePGPSignPolicy")
      });
    }
  }
  ...
  async function handlePGPEncryptPolicyChange(policy: string) {
    try {
      ...
      addToast({
        type: "error",
        message: $_("security.failedToUpdatePGPEncryptPolicy")
      });
    }
  }
  ...
  async function handleImportPGPRecipientKey() {
    ...
    pgpRecipientImportError = "";
    try {
      await ImportRecipientPGPKey(
        pgpRecipientImportEmail.trim(),
        pgpRecipientImportFilePath
      );
      addToast({
        type: "success",
        message: $_("security.recipientPGPKeyImported", {
          values: { email: pgpRecipientImportEmail.trim() }
        })
      });
  ...
  async function handleDeletePGPSenderKey(keyId: string) {
    try {
      ...
      addToast({
        type: "success",
        message: $_("security.recipientPGPKeyRemoved")
      });
      await loadData();
    } catch (err) {
      ...
      addToast({
        type: "error",
        message: $_("security.failedToRemoveRecipientPGPKey")
      });
    }
  }
  ...
  async function handleRemoveKeyServer(id: number) {
    try {
      ...
      addToast({
        type: "error",
        message: $_("security.failedToRemoveKeyServer")
      });
    }
  }
  ...
  async function handleKeyLookup() {
    ...
    try {
      ...
      if (armored) {
        addToast({
          type: "success",
          message: $_("security.pgpKeyFound", {
            values: { email: keyLookupEmail.trim() }
          })
        });
        keyLookupEmail = "";
        await loadData();
        ...
        addToast({
          type: "info",
          message: $_("security.pgpKeyNotFound", {
            values: { email: keyLookupEmail.trim() }
          })
        });
      }
  ...
  function mapImportError(
    err: unknown,
    type: "cert" | "key" | "recipientCert" | "recipientKey"
  ): string {
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`Import ${type} error:`, msg);
    if (msg.includes("does not match this account")) {
      return type.includes("key")
        ? $_("security.keyEmailMismatch")
        : $_("security.certEmailMismatch");
    }
    // Include the actual error detail for diagnosis
    const detail = msg.length > 0 ? `: ${msg}` : "";
    switch (type) {
      case "cert":
        return $_("security.certImportFailed") + detail;
      case "key":
        return $_("security.keyImportFailed") + detail;
      case "recipientCert":
        return $_("security.recipientCertImportFailed") + detail;
      case "recipientKey":
        return $_("security.recipientKeyImportFailed") + detail;
    }
  }
...
<div class="space-y-6">
  ...
  {:else}
    ...
    <div class="space-y-4">
      ...
      >
        <Icon
          icon={pgpCollapsed ? "mdi:chevron-right" : "mdi:chevron-down"}
        ...
        {#if pgpKeys.length > 0}
          <span
            ...
            >{$_("security.keysCount", {
              values: { count: pgpKeys.length }
            })}</span
          >
        {/if}
      </button>
      ...
      {#if !pgpCollapsed}
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <h4
              class="text-xs font-medium text-muted-foreground tracking-wider uppercase"
            >
              {$_("security.yourKeys")}
            </h4>
            <Button
              variant="outline"
              size="sm"
              onclick={handlePickAndImportPGP}
            >
          ...
          {#if pgpKeys.length === 0}
            <p class="text-sm text-muted-foreground py-2">
              {$_("security.noPGPKeysHelp")}
            </p>
          {:else}
            <div class="space-y-2">
              {#each pgpKeys as key}
                <div
                  class="gap-3 p-3 rounded-md border-border bg-card flex items-start border"
                >
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="gap-2 flex items-center">
                      ...
                      {#if key.isDefault}
                        <span
                          class="px-1.5 py-0.5 rounded bg-primary/10 text-primary font-medium text-[10px]"
                          >{$_("security.defaultBadge")}</span
                        >
                      {/if}
                      {#if key.isExpired}
                        <span
                          class="px-1.5 py-0.5 rounded bg-destructive/10 text-destructive font-medium text-[10px]"
                          >{$_("security.expiredBadge")}</span
                        >
                      {/if}
                    </div>
                    ...
                    <p class="text-xs text-muted-foreground mt-0.5">
                      {key.algorithm}{key.keySize ? ` ${key.keySize}-bit` : ""} &middot;
                      {key.fingerprint?.slice(-16)}
                    </p>
                    <p class="text-xs text-muted-foreground">
                      {$_("security.created")}
                      {formatDate(key.createdAtKey)}{key.expiresAtKey
                        ? ` · ${$_("security.expires")} ${formatDate(key.expiresAtKey)}`
                        : ""}
                    </p>
                  </div>
                  <div class="gap-1 flex flex-shrink-0 items-center">
                    {#if !key.isDefault}
                      <Button
                        variant="ghost"
                        size="sm"
                        onclick={() => handleSetDefaultPGP(key.id)}
                        title={$_("security.setAsDefault")}
                      >
        ...
        {#if pgpKeys.length > 0}
          ...
          <div class="space-y-2">
            ...
            <p class="text-xs text-muted-foreground">
              {$_("security.encryptRequiresRecipientKeys")}
            </p>
          </div>
        {/if}
        ...
        <div class="space-y-3">
          <button
            class="gap-2 text-xs font-medium text-muted-foreground tracking-wider hover:text-foreground flex w-full items-center text-left uppercase transition-colors"
            onclick={() => (keyServersCollapsed = !keyServersCollapsed)}
          >
            <Icon
              icon={keyServersCollapsed
                ? "mdi:chevron-right"
                : "mdi:chevron-down"}
          ...
          {#if !keyServersCollapsed}
                ...
                {#each keyServers as server}
                  ...
                  >
                    ...
                    <span class="text-sm flex-1 truncate"
                      >{server.url.replace("https://", "")}</span
                    >
                    <Button
                      variant="ghost"
                      size="sm"
                      onclick={() => handleRemoveKeyServer(server.id)}
                      title={$_("security.removeServer")}
                    >
            ...
            <div class="gap-2 flex items-center">
              <input
                ...
                placeholder="https://"
                class="px-3 py-1.5 rounded-md border-border bg-background text-sm focus:ring-primary flex-1 border focus:ring-2 focus:outline-none"
                onkeydown={(e) => {
                  if (e.key === "Enter") handleAddKeyServer();
                }}
              />
              <Button
                variant="outline"
                size="sm"
                onclick={handleAddKeyServer}
                disabled={addingKeyServer || !newKeyServerURL.trim()}
              >
                {#if addingKeyServer}
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <h4
              class="text-xs font-medium text-muted-foreground tracking-wider uppercase"
            >
              {$_("security.recipientKeys")}
            </h4>
            <Button
              variant="outline"
              size="sm"
              onclick={handlePickPGPRecipientKey}
            >
              <Icon icon="mdi:key-plus" class="w-4 h-4 mr-1" />
          ...
          <div class="gap-2 flex items-center">
            <input
              ...
              class="px-3 py-1.5 rounded-md border-border bg-background text-sm focus:ring-primary flex-1 border focus:ring-2 focus:outline-none"
              onkeydown={(e) => {
                if (e.key === "Enter") handleKeyLookup();
              }}
            />
            <Button
              variant="outline"
              ...
              disabled={keyLookupLoading || !keyLookupEmail.trim()}
            >
              {#if keyLookupLoading}
                <Icon icon="mdi:loading" class="w-4 h-4 animate-spin" />
              {:else}
          ...
          {#if pgpSenderKeys.length === 0}
            <p class="text-sm text-muted-foreground py-2">
              {$_("security.noRecipientPGPKeysHelp")}
            </p>
          {:else}
            <div class="space-y-2">
              {#each pgpSenderKeys as key}
                ...
                >
                  ...
                  />
                  <div class="min-w-0 flex-1">
                    <span class="text-sm block truncate">{key.email}</span>
                    <span class="text-xs text-muted-foreground block truncate"
                      >{key.fingerprint?.slice(-16)} &middot; {key.algorithm}</span
                    >
                  </div>
                  <span
                    class="px-1.5 py-0.5 rounded bg-muted text-muted-foreground font-medium flex-shrink-0 text-[10px]"
                  >
                    {key.source}
                  </span>
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onclick={() => handleDeletePGPSenderKey(key.id)}
                    title={$_("security.removeButton")}
                  >
                    <Icon icon="mdi:close" class="w-3.5 h-3.5" />
                  </Button>
                </div>
    ...
    <!-- S/MIME Section -->
    <div class="space-y-4 mt-8 pt-6 border-border border-t">
      <button
        class="gap-2 text-sm font-semibold text-foreground hover:text-primary flex w-full items-center text-left transition-colors"
        onclick={() => (smimeCollapsed = !smimeCollapsed)}
      >
        <Icon
          icon={smimeCollapsed ? "mdi:chevron-right" : "mdi:chevron-down"}
          class="w-4 h-4 flex-shrink-0"
        />
      ...
      {#if !smimeCollapsed}
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="gap-2 flex items-center">
                      ...
                      {#if cert.isSelfSigned}
                        <span
                          class="px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 dark:text-amber-400 font-medium text-[10px]"
                          >{$_("security.selfSignedBadge")}</span
                        >
                      {/if}
                    </div>
                    <p class="text-xs text-muted-foreground mt-0.5 truncate">
                    ...
                    <p class="text-xs text-muted-foreground mt-0.5">
                      {$_("security.issuerLabel")}
                      {cert.issuer}
                    </p>
                    <p class="text-xs text-muted-foreground">
                      {$_("security.validLabel")}
                  ...
                  <div class="gap-1 flex flex-shrink-0 items-center">
                    ...
                    <Button
                      ...
                      title={$_("security.removeCertificate")}
                    >
                      <Icon
                        icon="mdi:delete-outline"
                        class="w-4 h-4 text-destructive"
        ...
        {#if certificates.length > 0}
          ...
          <div class="space-y-2">
            ...
            <p class="text-xs text-muted-foreground">
              {$_("security.encryptRequiresRecipientCerts")}
            </p>
          </div>
        {/if}
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <h4
              class="text-xs font-medium text-muted-foreground tracking-wider uppercase"
            >
              {$_("security.recipientCertificates")}
            </h4>
            <Button
              variant="outline"
              size="sm"
              onclick={handlePickRecipientCert}
            >
              <Icon icon="mdi:certificate" class="w-4 h-4 mr-1" />
              {$_("security.importButton")}
            </Button>
          ...
          {#if senderCerts.length === 0}
            <p class="text-sm text-muted-foreground py-2">
              {$_("security.noRecipientCertsHelp")}
            </p>
          {:else}
            <div class="space-y-2">
              {#each senderCerts as cert}
                ...
                >
                  ...
                  />
                  <div class="min-w-0 flex-1">
                    <span class="text-sm block truncate">{cert.email}</span>
                    <span class="text-xs text-muted-foreground block truncate"
                      >{cert.subject}</span
                    >
                  </div>
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onclick={() => handleDeleteSenderCert(cert.id)}
                    title={$_("security.removeButton")}
                  >
                    <Icon icon="mdi:close" class="w-3.5 h-3.5" />
                  </Button>
...
<!-- Import Dialog -->
{#if showImportDialog}
  <div class="inset-0 fixed z-50 flex items-center justify-center">
    ...
    ></div>
    <div
      class="bg-background border-border rounded-lg shadow-xl p-6 max-w-md mx-4 relative w-full border"
    >
      <h3 class="text-lg font-semibold mb-4">
        {$_("security.importCertificateTitle")}
      </h3>

      <div class="space-y-4">
        <div>
          <p class="text-sm text-muted-foreground mb-1">
            {$_("security.fileLabel")}
          </p>
          <p class="text-sm font-mono bg-muted/50 px-3 py-2 rounded truncate">
            {getFileName(importFilePath)}
          </p>
        </div>
        ...
        <div>
          <label
            for="cert-password"
            class="text-sm text-muted-foreground mb-1 block"
          >
          ...
          <input
            ...
            class="px-3 py-2 rounded-md border-border bg-background text-sm focus:ring-primary w-full border focus:ring-2 focus:outline-none"
            onkeydown={(e) => {
              if (e.key === "Enter") handleImport();
            }}
          />
          <p class="text-xs text-muted-foreground mt-1">
            {$_("security.certificatePasswordHelp")}
          </p>
        </div>
      ...
      <div class="gap-2 mt-6 flex items-center justify-end">
        <Button
          variant="ghost"
          onclick={handleCancelImport}
          disabled={importing}
...
{#if showBERConfirmDialog}
  <div class="inset-0 fixed z-50 flex items-center justify-center">
    ...
    <div
      ...
      onkeydown={(e) => {
        if (e.key === "Escape") showBERConfirmDialog = false;
      }}
    ></div>
    <div
      class="bg-background border-border rounded-lg shadow-lg p-6 max-w-md relative w-full border"
    >
      <h3 class="text-lg font-semibold mb-4">
        {$_("security.berConversionTitle")}
      </h3>
      <p class="text-sm text-muted-foreground mb-6">
        {$_("security.berConversionMessage")}
      </p>
...
{#if showRecipientImportDialog}
  <div class="inset-0 fixed z-50 flex items-center justify-center">
    ...
    <div
      ...
      tabindex="-1"
      class="inset-0 bg-black/50 absolute"
      onclick={handleCancelRecipientImport}
      onkeydown={(e) => {
        if (e.key === "Escape") handleCancelRecipientImport();
      }}
    ></div>
    <div
      class="bg-background border-border rounded-lg shadow-xl p-6 max-w-md mx-4 relative w-full border"
    >
      <h3 class="text-lg font-semibold mb-4">
        {$_("security.importRecipientCertTitle")}
      </h3>
      ...
      <div class="space-y-4">
        <div>
          ...
          <p class="text-sm font-mono bg-muted/50 px-3 py-2 rounded truncate">
            {getFileName(recipientImportFilePath)}
          </p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            class="px-3 py-2 rounded-md border-border bg-background text-sm focus:ring-primary w-full border focus:ring-2 focus:outline-none"
            onkeydown={(e) => {
              if (e.key === "Enter") handleImportRecipientCert();
            }}
          />
          <p class="text-xs text-muted-foreground mt-1">
            {$_("security.recipientEmailHelp")}
          </p>
        </div>
...
{#if showPGPImportDialog}
  <div class="inset-0 fixed z-50 flex items-center justify-center">
    ...
    <div
      ...
      tabindex="-1"
      class="inset-0 bg-black/50 absolute"
      onclick={handleCancelPGPImport}
      onkeydown={(e) => {
        if (e.key === "Escape") handleCancelPGPImport();
      }}
    ></div>
    <div
      class="bg-background border-border rounded-lg shadow-xl p-6 max-w-md mx-4 relative w-full border"
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            class="px-3 py-2 rounded-md border-border bg-background text-sm focus:ring-primary w-full border focus:ring-2 focus:outline-none"
            onkeydown={(e) => {
              if (e.key === "Enter") handleImportPGP();
            }}
          />
          <p class="text-xs text-muted-foreground mt-1">
            {$_("security.keyPassphraseHelp")}
          </p>
        </div>
...
{#if showPGPRecipientImportDialog}
  <div class="inset-0 fixed z-50 flex items-center justify-center">
    ...
    <div
      ...
      tabindex="-1"
      class="inset-0 bg-black/50 absolute"
      onclick={handleCancelPGPRecipientImport}
      onkeydown={(e) => {
        if (e.key === "Escape") handleCancelPGPRecipientImport();
      }}
    ></div>
    <div
      class="bg-background border-border rounded-lg shadow-xl p-6 max-w-md mx-4 relative w-full border"
    >
      <h3 class="text-lg font-semibold mb-4">
        {$_("security.importRecipientPGPKeyTitle")}
      </h3>
      ...
      <div class="space-y-4">
        <div>
          ...
          <p class="text-sm font-mono bg-muted/50 px-3 py-2 rounded truncate">
            {getFileName(pgpRecipientImportFilePath)}
          </p>
        </div>

        <div>
          <label
            for="pgp-recipient-email"
            class="text-sm text-muted-foreground mb-1 block"
          >
            {$_("security.recipientEmailAddress")}
          </label>
          <input
            ...
            class="px-3 py-2 rounded-md border-border bg-background text-sm focus:ring-primary w-full border focus:ring-2 focus:outline-none"
            onkeydown={(e) => {
              if (e.key === "Enter") handleImportPGPRecipientKey();
            }}
          />
          <p class="text-xs text-muted-foreground mt-1">
            {$_("security.recipientEmailHelpKey")}
          </p>
        </div>
      ...
      <div class="gap-2 mt-6 flex items-center justify-end">
        <Button
          variant="ghost"
          onclick={handleCancelPGPRecipientImport}
          disabled={pgpRecipientImporting}
        >
          {$_("common.cancel")}
        </Button>
        <Button
          onclick={handleImportPGPRecipientKey}
          disabled={pgpRecipientImporting}
        >
