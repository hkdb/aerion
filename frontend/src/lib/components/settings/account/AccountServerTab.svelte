<script lang="ts">
  ...
  import { securityOptions, syncIntervalOptions } from "$lib/config/providers";
  ...
  async function handleFolderSubscriptionToggle(
    f: folder.Folder,
    subscribed: boolean
  ) {
  ...
  function getReadReceiptLabel(value: string): string {
    switch (value) {
      case "never":
        return $_("account.neverRequest");
      case "ask":
        return $_("account.askEachTime");
      case "always":
        return $_("account.alwaysRequest");
      default:
        return value;
    }
  }
  ...
  async function confirmRemoveCert() {
    ...
    try {
      ...
      trustedCerts = trustedCerts.filter(
        (c) => c.fingerprint !== confirmRemoveFingerprint
      );
    } catch (err) {
  ...
  const folderMappingTypes = [
    {
      key: "sent",
      labelKey: "account.folderSent",
      get: () => sentFolderPath || autoDetectedFolders["sent"] || "",
      set: (v: string) => {
        sentFolderPath = v;
        onFolderMappingChange("sent", v);
      }
    },
    {
      key: "drafts",
      labelKey: "account.folderDrafts",
      get: () => draftsFolderPath || autoDetectedFolders["drafts"] || "",
      set: (v: string) => {
        draftsFolderPath = v;
        onFolderMappingChange("drafts", v);
      }
    },
    {
      key: "trash",
      labelKey: "account.folderTrash",
      get: () => trashFolderPath || autoDetectedFolders["trash"] || "",
      set: (v: string) => {
        trashFolderPath = v;
        onFolderMappingChange("trash", v);
      }
    },
    {
      key: "spam",
      labelKey: "account.folderSpam",
      get: () => spamFolderPath || autoDetectedFolders["spam"] || "",
      set: (v: string) => {
        spamFolderPath = v;
        onFolderMappingChange("spam", v);
      }
    },
    {
      key: "archive",
      labelKey: "account.folderArchive",
      get: () => archiveFolderPath || autoDetectedFolders["archive"] || "",
      set: (v: string) => {
        archiveFolderPath = v;
        onFolderMappingChange("archive", v);
      }
    },
    {
      key: "all",
      labelKey: "account.folderAllMail",
      get: () => allMailFolderPath || autoDetectedFolders["all"] || "",
      set: (v: string) => {
        allMailFolderPath = v;
        onFolderMappingChange("all", v);
      }
    },
    {
      key: "starred",
      labelKey: "account.folderStarred",
      get: () => starredFolderPath || autoDetectedFolders["starred"] || "",
      set: (v: string) => {
        starredFolderPath = v;
        onFolderMappingChange("starred", v);
      }
    }
  ];
</script>
...
<div class="space-y-6">
    ...
    <div class="gap-3 grid grid-cols-2">
      <div class="space-y-2">
        ...
        <Input
          ...
          bind:value={imapHost}
          oninput={(e) =>
            onImapHostChange((e.target as HTMLInputElement).value)}
      ...
      <div class="gap-2 grid grid-cols-2">
        <div class="space-y-2">
          ...
          <Input
            ...
            bind:value={imapPort}
            oninput={(e) =>
              onImapPortChange(Number((e.target as HTMLInputElement).value))}
    ...
    <div class="gap-3 grid grid-cols-2">
      <div class="space-y-2">
        ...
        <Input
          ...
          bind:value={smtpHost}
          oninput={(e) =>
            onSmtpHostChange((e.target as HTMLInputElement).value)}
      ...
      <div class="gap-2 grid grid-cols-2">
        <div class="space-y-2">
          ...
          <Input
            ...
            bind:value={smtpPort}
            oninput={(e) =>
              onSmtpPortChange(Number((e.target as HTMLInputElement).value))}
  ...
  <div class="space-y-4">
    ...
    <div class="space-y-2">
      ...
      <Select.Root
        ...
        onValueChange={(v) => {
          readReceiptRequestPolicy = v;
          onReadReceiptPolicyChange(v);
        }}
      >
        <Select.Trigger>
          <Select.Value placeholder="Select">
  ...
  <div class="space-y-2">
    <button
      type="button"
      class="gap-2 text-sm font-medium hover:text-primary flex items-center transition-colors"
                  ...
                  <Select.Trigger class="h-9">
                    <Select.Value placeholder={$_("account.none")}>
                      {(mapping.get() || $_("account.none")) +
                        (autoDetectedFolders[mapping.key] === mapping.get()
                          ? " " + $_("account.detected")
                          : "")}
                    </Select.Value>
                  </Select.Trigger>
                  <Select.Content>
                    ...
                    {#each availableFolders as f (f.path)}
                      <Select.Item
                        value={f.path}
                        label={f.path +
                          (autoDetectedFolders[mapping.key] === f.path
                            ? " " + $_("account.detected")
                            : "")}
                      />
                    {/each}
  ...
  <div class="space-y-2">
    ...
    {#if showFolderSync}
      <div class="space-y-4 pl-6 pt-2 border-border ml-2 border-l">
        ...
        <div class="space-y-1">
          <div class="gap-3 flex items-center">
            ...
            <Switch
              ...
              onCheckedChange={(v) => {
                syncFoldersEnabled = v;
                onSyncFoldersEnabledChange(v);
                if (v) loadSyncFolders();
              }}
            />
          </div>
          <p class="text-xs text-muted-foreground">
            ...
            {:else}
              <div class="space-y-1 max-h-48 overflow-y-auto">
                {#each syncFolders as f (f.id)}
                  ...
                  <label
                    class="gap-2 py-0.5 flex cursor-pointer items-center {isCore
                      ? 'opacity-60'
                      : ''}"
                  >
                    <input
                      ...
                      disabled={isCore}
                      onchange={(e) =>
                        handleFolderSubscriptionToggle(
                          f,
        ...
        {:else}
          <div class="space-y-3">
            {#each trustedCerts as cert (cert.fingerprint)}
              <div
                class="gap-3 rounded-lg bg-muted/30 p-3 flex items-start justify-between border"
              >
                <div class="space-y-1 min-w-0">
                  ...
                  <div class="text-xs text-muted-foreground space-y-0.5 pl-6">
                    <p>
                      {$_("account.certFingerprint")}
                      <span class="font-mono"
                        >{formatFingerprint(cert.fingerprint)}</span
                      >
                    </p>
                    <p>
                      {$_("account.certExpires")}
                      {formatCertDate(cert.notAfter)}
                    </p>
                  </div>
                </div>
                <Button
