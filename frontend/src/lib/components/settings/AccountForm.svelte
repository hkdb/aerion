<script lang="ts">
  ...
  import {
    AcceptCertificate,
    GetAccountFoldersForMapping,
    GetAutoDetectedFolders,
    GetIdentities
  ...
  function getSyncIntervalLabel(value: string): string {
    const numValue = Number(value);
    return (
      syncIntervalOptions.find((opt) => opt.value === numValue)?.label ||
      `${value} min`
    );
  }
  ...
  function getReadReceiptLabel(value: string): string {
    return (
      readReceiptRequestOptions.find((opt) => opt.value === value)?.label ||
      value
    );
  }
  ...
  const folderMappingTypes = $derived([
    {
      key: "sent",
      label: $_("account.folderSent"),
      getValue: () => sentFolderPath,
      setValue: (v: string) => (sentFolderPath = v)
    },
    {
      key: "drafts",
      label: $_("account.folderDrafts"),
      getValue: () => draftsFolderPath,
      setValue: (v: string) => (draftsFolderPath = v)
    },
    {
      key: "trash",
      label: $_("account.folderTrash"),
      getValue: () => trashFolderPath,
      setValue: (v: string) => (trashFolderPath = v)
    },
    {
      key: "spam",
      label: $_("account.folderSpam"),
      getValue: () => spamFolderPath,
      setValue: (v: string) => (spamFolderPath = v)
    },
    {
      key: "archive",
      label: $_("account.folderArchive"),
      getValue: () => archiveFolderPath,
      setValue: (v: string) => (archiveFolderPath = v)
    },
    {
      key: "all",
      label: $_("account.folderAllMail"),
      getValue: () => allMailFolderPath,
      setValue: (v: string) => (allMailFolderPath = v)
    },
    {
      key: "starred",
      label: $_("account.folderStarred"),
      getValue: () => starredFolderPath,
      setValue: (v: string) => (starredFolderPath = v)
    }
  ]);
  ...
  async function loadFoldersForMapping() {
    ...
    try {
      ...
      sentFolderPath =
        editAccount.sentFolderPath || autoDetectedFolders.sent || "";
      // @ts-ignore
      draftsFolderPath =
        editAccount.draftsFolderPath || autoDetectedFolders.drafts || "";
      // @ts-ignore
      trashFolderPath =
        editAccount.trashFolderPath || autoDetectedFolders.trash || "";
      // @ts-ignore
      spamFolderPath =
        editAccount.spamFolderPath || autoDetectedFolders.spam || "";
      // @ts-ignore
      archiveFolderPath =
        editAccount.archiveFolderPath || autoDetectedFolders.archive || "";
      // @ts-ignore
      allMailFolderPath =
        editAccount.allMailFolderPath || autoDetectedFolders.all || "";
      // @ts-ignore
      starredFolderPath =
        editAccount.starredFolderPath || autoDetectedFolders.starred || "";
    } catch (err) {
  ...
  function getOAuthButtonText(provider: EmailProvider | null): string {
    ...
    const oauthType = getOAuthProviderType(provider);
    if (oauthType === "google")
      return $_("account.signInWith", { values: { provider: "Google" } });
    if (oauthType === "microsoft")
      return $_("account.signInWith", { values: { provider: "Microsoft" } });
  ...
  function validate(): boolean {
    ...
    if (!email.trim()) errors.email = $_("account.emailRequired");
      ...
      errors.email = $_("account.invalidEmail");
  ...
  async function handleTestConnection() {
    ...
    try {
      ...
      if (result.success) {
        testResult = {
          success: true,
          message: $_("account.connectionSuccessful")
        };
        ...
        testResult = {
          success: false,
          message: result.error || $_("account.connectionFailed")
        };
      }
    } catch (err) {
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      if (
        authMethod === "oauth2" &&
        oauthStore.isFlowSuccess &&
        oauthStore.flowResult
      ) {
    ...
    <div class="space-y-4">
      ...
      <div class="gap-3 grid grid-cols-3">
        {#each providers as provider (provider.id)}
          <button
            type="button"
            class="gap-2 p-4 rounded-lg border-input bg-background hover:bg-accent hover:text-accent-foreground flex flex-col items-center border transition-colors"
  ...
  {:else}
    ...
    <div class="space-y-4">
      {#if !editAccount}
        <button
          type="button"
          class="gap-1 text-sm text-muted-foreground hover:text-foreground flex items-center transition-colors"
      ...
      {#if selectedProvider?.notes}
        <div
          class="gap-2 p-3 rounded-lg bg-amber-500/10 border-amber-500/20 flex items-start border"
        >
          <Icon
      ...
      <div class="gap-4 grid">
        ...
        <div class="space-y-3">
            ...
            <div class="space-y-3">
              ...
              {#if allowsPasswordFallback(selectedProvider!)}
                ...
                <div class="gap-2 flex">
                  ...
                  >
                    <Icon
                      icon={getOAuthButtonIcon(selectedProvider)}
                      class="w-4 h-4 mr-2"
                    />
                    OAuth
                  </Button>
              ...
              {#if authMethod === "oauth2"}
                <!-- OAuth Flow UI -->
                <div class="rounded-lg border-border p-4 space-y-3 border">
                  ...
                  {:else if oauthStore.flowState === "pending"}
                    ...
                    <div class="gap-3 py-2 flex flex-col items-center">
                      <Icon
                        icon="mdi:loading"
                  ...
                  {:else if oauthStore.flowState === "success"}
                    ...
                    <div class="gap-3 py-2 flex items-center">
                      <div
                        class="w-10 h-10 bg-green-500/10 flex flex-shrink-0 items-center justify-center rounded-full"
                      >
                  ...
                  {:else if oauthStore.flowState === "error"}
                    ...
                    <div class="space-y-3">
                      <div class="gap-3 flex items-start">
                        <div
                          class="w-10 h-10 bg-destructive/10 flex flex-shrink-0 items-center justify-center rounded-full"
                        >
                          <Icon
                            icon="mdi:alert"
          ...
          {:else if editAccount && editAccount.authType === "oauth2"}
            ...
            <div class="space-y-2">
              <Label>{$_("account.authentication")}</Label>
              <div class="rounded-lg border-border p-4 border">
                <div class="gap-3 flex items-center">
                  <div
                    class="w-10 h-10 bg-primary/10 flex flex-shrink-0 items-center justify-center rounded-full"
                  >
                    <Icon
                      icon={getOAuthButtonIcon(selectedProvider)}
                      class="w-5 h-5"
          ...
          {:else}
            ...
            <div class="space-y-2">
              <Label for="password">
                {selectedProvider?.notes?.includes("App Password")
                  ? $_("account.appPassword")
                  : $_("account.password")}
              </Label>
              <Input
                id="password"
                type="password"
                placeholder={editAccount
                  ? $_("account.leaveEmptyToKeep")
                  : $_("account.password")}
                bind:value={password}
                class={errors.password ? "border-destructive" : ""}
              />
      ...
      {#if showAdvanced}
        <div class="space-y-4 pt-2 border-border border-t">
          <!-- IMAP Settings -->
          <div class="space-y-3">
            ...
            >
              ...
              {#if !editAccount}
                <span class="text-xs text-muted-foreground"
                  >{$_("account.saveAccountFirst")}</span
                >
              {/if}
            </button>

            {#if showFolderMapping}
              <div class="space-y-3 pl-6 pt-2 border-border ml-2 border-l">
                <p class="text-xs text-muted-foreground">
                ...
                {:else if availableFolders.length === 0}
                  <p class="text-sm text-muted-foreground">
                    {$_("account.noFoldersAvailable")}
                  </p>
                {:else}
                  <div class="gap-3 grid">
                    <!-- Sent -->
                    <div class="gap-2 grid grid-cols-[100px_1fr] items-center">
                        ...
                        <Select.Content>
                          ...
                          {#each availableFolders as f (f.path)}
                            <Select.Item
                              value={f.path}
                              label={f.path +
                                (autoDetectedFolders.sent === f.path
                                  ? " " + $_("account.detected")
                                  : "")}
                            />
                          {/each}
                        ...
                        <Select.Content>
                          ...
                          {#each availableFolders as f (f.path)}
                            <Select.Item
                              value={f.path}
                              label={f.path +
                                (autoDetectedFolders.drafts === f.path
                                  ? " " + $_("account.detected")
                                  : "")}
                            />
                          {/each}
                        ...
                        <Select.Content>
                          ...
                          {#each availableFolders as f (f.path)}
                            <Select.Item
                              value={f.path}
                              label={f.path +
                                (autoDetectedFolders.trash === f.path
                                  ? " " + $_("account.detected")
                                  : "")}
                            />
                          {/each}
                        ...
                        <Select.Content>
                          ...
                          {#each availableFolders as f (f.path)}
                            <Select.Item
                              value={f.path}
                              label={f.path +
                                (autoDetectedFolders.spam === f.path
                                  ? " " + $_("account.detected")
                                  : "")}
                            />
                          {/each}
                        ...
                        <Select.Content>
                          ...
                          {#each availableFolders as f (f.path)}
                            <Select.Item
                              value={f.path}
                              label={f.path +
                                (autoDetectedFolders.archive === f.path
                                  ? " " + $_("account.detected")
                                  : "")}
                            />
                          {/each}
                        ...
                        <Select.Content>
                          ...
                          {#each availableFolders as f (f.path)}
                            <Select.Item
                              value={f.path}
                              label={f.path +
                                (autoDetectedFolders.all === f.path
                                  ? " " + $_("account.detected")
                                  : "")}
                            />
                          {/each}
                        ...
                        <Select.Content>
                          ...
                          {#each availableFolders as f (f.path)}
                            <Select.Item
                              value={f.path}
                              label={f.path +
                                (autoDetectedFolders.starred === f.path
                                  ? " " + $_("account.detected")
                                  : "")}
                            />
                          {/each}
      ...
      {#if testResult}
        <div
          class="gap-2 p-3 rounded-lg flex items-start {testResult.success
        ...
        >
          <Icon
            ...
            class="w-5 h-5 mt-0.5 flex-shrink-0 {testResult.success
              ? 'text-green-500'
              : 'text-destructive'}"
          />
          <p
      ...
      <!-- Actions -->
      <div
        class="pt-4 border-border flex items-center justify-between border-t"
      >
        <Button
          type="button"
          variant="outline"
