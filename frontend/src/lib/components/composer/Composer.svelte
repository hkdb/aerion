<script lang="ts">
  ...
  import { account, app, contact, smtp } from "../../../../wailsjs/go/models";
      ...
      try {
        ...
        for (const email of missingEmails) {
          try {
            ...
            if (armored) {
              recipientPGPKeyStatus = {
                ...recipientPGPKeyStatus,
                [email]: true
              };
            }
          } catch {
            /* silent — lookup failure is not an error for the user */
          }
        }
      } catch (err) {
  ...
  async function handleImportRecipientCert() {
    try {
      ...
      if (missingCertRecipients.length > 0) {
        ...
        addToast({
          type: "success",
          message: $_("composer.certImported", {
            values: { email: missingCertRecipients[0] }
          })
        });
        // Re-check certs
        const allEmails = [...toRecipients, ...ccRecipients, ...bccRecipients]
  ...
  async function handleImportRecipientPGPKey() {
    try {
      ...
      if (missingPGPKeyRecipients.length > 0) {
        ...
        addToast({
          type: "success",
          message: $_("composer.pgpKeyImported", {
            values: { email: missingPGPKeyRecipients[0] }
          })
        });
        const allEmails = [...toRecipients, ...ccRecipients, ...bccRecipients]
          .map((r) => r.address)
  ...
  let draftStatusLabel = $derived.by(() => {
    if (saveStatus === "saving")
      return encryptMessage || pgpEncryptMessage
        ? $_("composer.encrypting")
        : $_("composer.saving");
    ...
    if (encryptMessage || pgpEncryptMessage) {
      switch (syncStatus) {
        case "synced":
          return $_("composer.encryptedSynced");
        case "pending":
          return $_("composer.encryptedDraft");
        case "failed":
          return $_("composer.encryptedOffline");
        default:
          return "";
      }
    }
    switch (syncStatus) {
      case "synced":
        return $_("composer.synced");
      case "pending":
        return $_("composer.savedLocally");
      case "failed":
        return $_("composer.savedLocallyOffline");
      default:
        return "";
    }
  });
  ...
  function getUserComposedText(): string {
    ...
    if (isPlainTextMode) {
      ...
      if (cutoff.length > 0)
        return plainTextContent.substring(0, Math.min(...cutoff));
    ...
    const userHtml =
      cutoffs.length > 0 ? html.substring(0, Math.min(...cutoffs)) : html;
  ...
  function getDisplayMode(): "new" | "reply" | "reply-all" | "forward" {
    ...
    if (initialMessage.in_reply_to) {
      ...
      if (
        (initialMessage.to?.length || 0) > 1 ||
        (initialMessage.cc?.length || 0) > 0
      ) {
        return "reply-all";
      }
      return "reply";
  ...
  function hasContent(): boolean {
    const bodyText = isPlainTextMode
  ...
  function buildMessage(): smtp.ComposeMessage {
    ...
    const smtpAttachments: smtp.Attachment[] = attachments.map(
      (att) =>
        new smtp.Attachment({
  ...
  function getContentHash(): string {
    const bodyContent = isPlainTextMode
      ? plainTextContent
      : editor?.getHTML() || "";
  ...
  async function saveDraft() {
    ...
    savingComplete = new Promise<void>((resolve) => {
  ...
  onMount(async () => {
    ...
    try {
      ...
      const accountIdentities = identities.filter(
        (i) => i.accountId === accountId
      );
      const defaultIdentity =
        accountIdentities.find((i) => i.isDefault) || accountIdentities[0];
      const selectedIdentity =
        matchedIdentity || defaultIdentity || identities[0];
      if (selectedIdentity) {
    ...
    setTimeout(() => {
      ...
      if (identity) {
        ...
        const fwdIdx = content.indexOf(
          "---------- Forwarded message ----------"
        );
        const boundaries = [quoteIdx, wroteIdx, fwdIdx].filter((i) => i > -1);
        const quoteBoundary =
          boundaries.length > 0 ? Math.min(...boundaries) : content.length;
  ...
  function selectIdentityForReply(): account.Identity | null {
    ...
    const fromEmail = (
      (initialMessage.from as any)?.address ||
      (initialMessage.from as any)?.email ||
      ""
    ).toLowerCase();
    ...
    return (
      identities.find(
        (identity) => identity.email.toLowerCase() === fromEmail
      ) || null
    );
  }
  ...
  async function loadSMIMEForEmail(email: string) {
    ...
    smimeCertFingerprint = cert.fingerprint
      ? cert.fingerprint.substring(0, 8).toUpperCase()
      : "";
  ...
  async function updateSecurityForIdentity(email: string) {
    ...
    try {
      await loadSMIMEForEmail(email);
    } catch (err) {
      console.error("Failed to load S/MIME settings:", err);
    }

    try {
      await loadPGPForEmail(email);
    } catch (err) {
  ...
  function handleIdentityChange(newIdentityId: string) {
    ...
    if (newIdentity.accountId !== oldAccountId) {
      api
        .getAccount(newIdentity.accountId)
        .then((acc) => {
          applyReadReceiptPolicy(acc.readReceiptRequestPolicy || "never");
        })
        .catch((err) => {
  ...
  function initializeFromMessage() {
    ...
    if (initialMessage.attachments?.length > 0) {
      for (const att of initialMessage.attachments) {
        const base64Data =
          att.content_base64 || (att.content as unknown as string);
    ...
    if (htmlBody.includes("data-original-src")) {
      ...
      if (
        getAlwaysLoadImages() ||
        imagesLoaded ||
        (senderEmail && isImageAllowedSync(senderEmail))
      ) {
  ...
  function validateBeforeSend(): boolean {
    ...
    if (encryptMessage && missingCertRecipients.length > 0) {
      addToast({
        type: "error",
        message: $_("composer.cannotEncryptMissingCert", {
          values: { emails: missingCertRecipients.join(", ") }
        })
      });
      return false;
    }
    ...
    if (pgpEncryptMessage && missingPGPKeyRecipients.length > 0) {
      addToast({
        type: "error",
        message: $_("composer.cannotEncryptMissingPGPKey", {
          values: { emails: missingPGPKeyRecipients.join(", ") }
        })
      });
      return false;
    }
  ...
  async function handleInlineImageFile(file: File) {
    ...
    try {
      ...
      const inlineImage: InlineImage = {
        ...
        filename:
          file.name ||
          `image${inlineImageCounter}.${contentType.split("/")[1] || "png"}`
      };
      ...
      editor
        ?.chain()
        .focus()
        .setImage({ src: dataUrl, alt: inlineImage.filename })
        .run();
      ...
      try {
        ...
        if (att.contentType.startsWith("image/")) {
          ...
          editor
            ?.chain()
            .focus()
            .setImage({ src: dataUrl, alt: att.filename })
            .run();
          continue;
        }
        ...
        if (att.size > MAX_ATTACHMENT_SIZE) {
          addToast({
            type: "error",
            message: $_("composer.attachmentTooLarge")
          });
          continue;
        }
        attachments = [
...
>
  <!-- Header -->
  <div
    class="px-4 py-3 border-border flex items-center justify-between border-b"
  >
    <div class="gap-3 flex items-center">
      ...
      {#if draftStatusLabel}
        <span class="text-xs text-muted-foreground gap-1 flex items-center">
          <Icon
            icon={draftStatusIcon}
            class="w-3 h-3 {draftStatusColor} {saveStatus === 'saving'
              ? 'animate-spin'
              : ''}"
          />
          {draftStatusLabel}
    ...
    <div class="gap-2 flex items-center">
      ...
      {#if !isDetached && api.openComposerWindow}
        <button
          ...
          class="p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-md transition-colors disabled:cursor-not-allowed disabled:opacity-50"
      ...
      <button
        ...
        class="px-4 py-1.5 text-sm font-medium text-primary-foreground bg-primary hover:bg-primary/90 rounded-md gap-2 flex items-center transition-colors disabled:cursor-not-allowed disabled:opacity-50"
      >
        {#if sending}
          <Icon icon="mdi:loading" class="w-4 h-4 animate-spin" />
          {$_("composer.sending")}
  ...
  <div class="min-h-0 flex flex-1 flex-col overflow-hidden">
    <!-- From -->
    <div class="gap-2 px-4 py-2 border-border flex items-center border-b">
      <span class="text-sm text-muted-foreground w-16"
        >{$_("composer.from")}:</span
      >
      <div class="flex-1">
        <Select.Root
        ...
        >
          <Select.Trigger
            class="h-8 border-0 bg-transparent shadow-none focus:ring-0"
          >
              ...
              {#if selectedIdentityId}
                ...
                {#if identity}
                  {@const group = allGroups.find(
                    (g) => g.account?.id === identity.accountId
                  )}
                  {#if group?.account?.color}
                    <span
                      class="w-2 h-2 mr-1.5 inline-block flex-shrink-0 rounded-full"
                      style="background-color: {group.account.color}"
                    ></span>
                  {/if}
          ...
          <Select.Content>
            {#if allGroups.length > 0}
              ...
              {#each allGroups as group (group.account?.id)}
                <Select.Group>
                  <Select.GroupHeading
                    class="gap-1.5 px-2 py-1 text-xs font-medium text-muted-foreground flex items-center"
                  >
                    {#if group.account?.color}
                      <span
                        class="w-2 h-2 inline-block flex-shrink-0 rounded-full"
                        style="background-color: {group.account.color}"
                      ></span>
                    {/if}
    ...
    <!-- To -->
    <div class="gap-2 px-4 py-2 border-border flex items-start border-b">
      <span class="text-sm text-muted-foreground w-16 pt-1"
        >{$_("composer.to")}:</span
      >
      <div class="flex-1">
        <RecipientInput
      ...
      {#if !showCc || !showBcc}
        <div class="gap-1 text-sm text-muted-foreground flex items-center">
          {#if !showCc}
            <button
              onclick={() => (showCc = true)}
              class="hover:text-foreground">{$_("composer.cc")}</button
            >
          {/if}
          {#if !showBcc}
            <button
              onclick={() => (showBcc = true)}
              class="hover:text-foreground">{$_("composer.bcc")}</button
            >
          {/if}
        </div>
      {/if}
    ...
    <!-- Subject -->
    <div class="gap-2 px-4 py-2 border-border flex items-center border-b">
      <label for="composer-subject" class="text-sm text-muted-foreground w-16"
        >{$_("composer.subject")}:</label
      >
      <input
        id="composer-subject"
        bind:value={subject}
        ...
        class="text-sm flex-1 bg-transparent focus:outline-none"
        onkeydown={(e) => {
    ...
    {#if showPGPSignOption || showPGPEncryptOption}
      <div
        class="px-4 py-3.5 border-border text-xs flex items-center border-b {securityMode ===
        'pgp'
          ? 'bg-muted/50'
          : ''}"
      >
        ...
        <div class="gap-3 ml-auto flex items-center">
          {#if securityMode === "pgp"}
            <span class="text-muted-foreground"
              >{$_("composer.securityModeHint")}</span
            >
          {/if}
          {#if showPGPSignOption}
            <div
              class="gap-1.5 flex items-center"
              title={$_("composer.pgpSign")}
            >
              <span>{$_("composer.sign")}</span>
              <Switch
                bind:checked={pgpSignMessage}
                onCheckedChange={(v) => {
                  if (v) {
                    signMessage = false;
                  }
          ...
          {#if showPGPEncryptOption}
            <div
              class="gap-1.5 flex items-center"
              title={$_("composer.pgpEncrypt")}
            >
              <span>{$_("composer.encrypt")}</span>
              <Switch
                bind:checked={pgpEncryptMessage}
                onCheckedChange={(v) => {
                  if (v) {
                    encryptMessage = false;
                  }
      ...
      >
        ...
        <div class="gap-3 ml-auto flex items-center">
          ...
          {#if showSignOption}
            <div
              class="gap-1.5 flex items-center"
              title={$_("composer.smimeSign")}
            >
              <span>{$_("composer.sign")}</span>
              <Switch
                bind:checked={signMessage}
                onCheckedChange={(v) => {
                  if (v) {
                    pgpSignMessage = false;
                  }
          ...
          {#if showEncryptOption}
            <div
              class="gap-1.5 flex items-center"
              title={$_("composer.smimeEncrypt")}
            >
              <span>{$_("composer.encrypt")}</span>
              <Switch
                bind:checked={encryptMessage}
                onCheckedChange={(v) => {
                  if (v) {
                    pgpEncryptMessage = false;
                  }
    ...
    {#if composerImagesBlocked}
      <div
        class="gap-2 px-3 py-2 mx-2 mt-2 rounded-md bg-yellow-500/10 border-yellow-500/30 text-sm flex items-center border"
      >
        <Icon
          icon="mdi:image-off"
        ...
        >
        <button
          class="px-2 py-1 text-xs font-medium rounded bg-yellow-600 text-white hover:bg-yellow-700 ml-auto transition-colors"
    ...
    <!-- Editor -->
    <div class="bg-white dark:bg-zinc-900 flex-1 overflow-auto">
      {#if isPlainTextMode}
        <textarea
          bind:value={plainTextContent}
          placeholder={$_("composer.writePlaceholder")}
          class="p-3 font-mono text-sm h-full w-full resize-none bg-transparent focus:outline-none"
    ...
    {#if encryptMessage && missingCertRecipients.length > 0}
      <div
        class="gap-2 text-xs px-3 py-1.5 bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-300 flex items-center border-t"
      >
        <Icon icon="mdi:alert" class="w-3.5 h-3.5 flex-shrink-0" />
        <span class="flex-1"
          >{$_("composer.noCertFor", {
            values: { emails: missingCertRecipients.join(", ") }
          })}</span
        >
        <button
          onclick={handleImportRecipientCert}
          class="px-2 py-0.5 rounded bg-amber-200 dark:bg-amber-800 hover:bg-amber-300 dark:hover:bg-amber-700 font-medium transition-colors"
          >{$_("composer.import")}</button
        >
        <button
          onclick={() => (encryptMessage = false)}
          class="px-2 py-0.5 rounded hover:bg-amber-200 dark:hover:bg-amber-800 font-medium transition-colors"
    ...
    {#if pgpEncryptMessage && missingPGPKeyRecipients.length > 0}
      ...
      >
        <Icon icon="mdi:alert" class="w-3.5 h-3.5 flex-shrink-0" />
        <span class="flex-1"
          >{$_("composer.noPGPKeyFor", {
            values: { emails: missingPGPKeyRecipients.join(", ") }
          })}</span
        >
        <button
          onclick={handleImportRecipientPGPKey}
          class="px-2 py-0.5 rounded bg-amber-200 dark:bg-amber-800 hover:bg-amber-300 dark:hover:bg-amber-700 font-medium transition-colors"
          >{$_("composer.import")}</button
        >
        <button
          onclick={() => (pgpEncryptMessage = false)}
          class="px-2 py-0.5 rounded hover:bg-amber-200 dark:hover:bg-amber-800 font-medium transition-colors"
          >{$_("common.cancel")}</button
        >
      </div>
    ...
    <!-- Footer -->
    <div
      class="gap-2 px-4 py-2 border-border text-sm text-muted-foreground flex items-center border-t"
    >
      <button
        onclick={handleAttachFiles}
        class="gap-1 hover:text-foreground flex items-center transition-colors"
      >
      ...
      {#if showReadReceiptOption}
        <label
          class="gap-1.5 text-xs hover:text-foreground flex cursor-pointer items-center transition-colors"
        >
          <input
            type="checkbox"
  ...
  {#if isDraggingOver}
    <div
      class="inset-0 bg-primary/10 pointer-events-none absolute z-10 flex items-center justify-center"
    >
      <div
        class="bg-background border-primary rounded-lg px-8 py-6 border-2 border-dashed text-center"
      >
        <Icon
          icon="mdi:attachment"
          class="w-12 h-12 text-primary mb-2 mx-auto"
        />
...
<AlertDialog.Root bind:open={showEmptySubjectDialog}>
  <AlertDialog.Content>
    ...
    <AlertDialog.Footer>
      ...
      <AlertDialog.Action onclick={handleConfirmEmptySubject}
        >{$_("composer.sendAnywayGeneric")}</AlertDialog.Action
      >
    </AlertDialog.Footer>
  </AlertDialog.Content>
</AlertDialog.Root>
...
<AlertDialog.Root bind:open={showMissingAttachmentDialog}>
  <AlertDialog.Content>
    <AlertDialog.Header>
      <AlertDialog.Title
        >{$_("composer.missingAttachmentTitle")}</AlertDialog.Title
      >
      <AlertDialog.Description>
    ...
    <AlertDialog.Footer>
      <AlertDialog.Cancel>{$_("common.cancel")}</AlertDialog.Cancel>
      <AlertDialog.Action onclick={handleConfirmMissingAttachment}
        >{$_("composer.sendAnywayGeneric")}</AlertDialog.Action
      >
    </AlertDialog.Footer>
  </AlertDialog.Content>
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        ...
        <code
          class="bg-muted px-3 py-2 rounded text-sm font-mono mb-3 block overflow-x-auto select-all"
          >flatpak override --user --filesystem=home io.github.hkdb.Aerion</code
        >
        <p class="mb-3 text-sm text-destructive">
          {$_("composer.flatpakDndSecurityWarning")}
        </p>
        <p class="text-sm text-muted-foreground">
          {$_("composer.flatpakDndAlternative")}
        </p>
      </AlertDialog.Description>
    </AlertDialog.Header>
    <AlertDialog.Footer>
      <AlertDialog.Action onclick={() => (showFlatpakDndDialog = false)}
        >{$_("common.ok")}</AlertDialog.Action
      >
    </AlertDialog.Footer>
  </AlertDialog.Content>
</AlertDialog.Root>
