<script lang="ts">
  ...
  const contextMenuMessageIds = $derived(
    useMultiSelect ? selectedMessageIds : ownMessageIds
  );
  const contextMenuIsStarred = $derived(
    useMultiSelect ? selectedIsStarred : ownIsStarred
  );
  const contextMenuIsRead = $derived(
    useMultiSelect ? selectedIsRead : ownIsRead
  );
</script>
...
<MessageContextMenu
  ...
  onOpenChange={(open: boolean) => {
    if (open) handleContextMenu();
  }}
>
  <div
    data-conversation-row
    class="group flex w-full items-start {densityClasses.row[
      density
    ]} border-border cursor-pointer border-b text-left transition-colors duration-300 outline-none {selected
      ? 'bg-primary/20'
      : 'hover:bg-muted/50'} {getAccentBarUnread() && hasUnread
  ...
  >
    ...
    <div
      class="{densityClasses.checkbox[
        density
      ]} flex flex-shrink-0 items-center justify-center self-center {checked
        ? 'opacity-100'
    ...
    >
      ...
      >
        {#if checked}
          <Icon
            icon="mdi:check"
            class="{densityClasses.checkIcon[density]} text-primary-foreground"
          />
        {/if}
      </button>
    </div>
    ...
    <!-- Avatar -->
    <div
      class="{densityClasses.avatar[
        density
      ]} text-white font-medium flex flex-shrink-0 items-center justify-center rounded-full {getAvatarColor(
        conversation
      )}"
    >
    ...
    <div class="min-w-0 flex-1">
      <div class="gap-2 mb-0.5 flex items-center">
        ...
        {#if highlightedFromName}
          <span
            class="{densityClasses.senderText[density]} truncate {hasUnread
        ...
        {#if conversation.messageCount > 1}
          <span
            class="flex-shrink-0 {densityClasses.badge[
              density
        ...
        {#if isSearchResult && searchFolderName}
          <span
            class="flex-shrink-0 {densityClasses.badge[
              density
            ]} rounded bg-muted/50 text-muted-foreground gap-1 flex items-center"
            title={$_("messageList.foundIn", {
              values: { folder: searchFolderName }
            })}
          >
          ...
          {#if isNonLocal}
            <span title={$_("search.notSyncedLocally")}>
              <Icon
                ...
                class="{densityClasses.icon[density]} text-muted-foreground"
              />
            </span>
          {/if}
      ...
      {#if highlightedSubject}
        <p
          class="truncate {densityClasses.text[density]} {hasUnread
      ...
      {#if highlightedSnippet}
        <p
          class="truncate {densityClasses.text[density]} text-muted-foreground"
        >
      ...
      {:else if conversation.isEncrypted}
        <p
          class="truncate {densityClasses.text[
            density
    ...
    <!-- Star -->
    <button
      class="p-1 -mr-1 rounded hover:bg-muted flex-shrink-0 transition-colors duration-200"
      onclick={handleStarClick}
    >
      <Icon
        icon={conversation.isStarred ? "mdi:star" : "mdi:star-outline"}
        class="{densityClasses.starIcon[density]} {conversation.isStarred
          ? 'text-yellow-500'
          : 'text-muted-foreground'}"
      />
    </button>
