<script lang="ts">
  ...
  const syncing = $derived(
    !!(
      accountId &&
      folderId &&
      accountStore.syncProgress[accountId]?.[folderId] !== undefined
    )
  );
  ...
  const filterLabel = $derived(
    (() => {
      switch (filterMode) {
        case "unread":
          return $_("messageList.filterUnread");
        case "starred":
          return $_("messageList.filterStarred");
        case "attachments":
          return $_("messageList.filterAttachments");
        default:
          return "";
      }
    })()
  ...
  onMount(() => {
    EventsOn(
      ...
      (data: { accountId: string; folderId: string }) => {
        ...
        if (
          (isUnifiedView && isInboxFolder(data.accountId, data.folderId)) ||
          (!isUnifiedView &&
            accountId &&
            folderId &&
    ...
    EventsOn(
      ...
      (data: { messageIds: string[]; isRead: boolean }) => {
        ...
        for (const c of conversations) {
          const affectedCount = (c.messageIds || []).filter((id) =>
            data.messageIds.includes(id)
          ).length;
          if (affectedCount > 0) {
            anyUpdated = true;
    ...
    try {
      ...
      if (status) {
        ...
        if (status.totalCount > 0) {
          indexProgress = Math.round(
            (status.indexedCount / status.totalCount) * 100
          );
        }
      }
  ...
  const selectedMessageIds = $derived([
    ...new Set(
      [...conversations, ...searchResults]
        ...
        .flatMap(
          (c: any) => c.messageIds || c.messages?.map((m: any) => m.id) || []
        )
    )
  ]);
    ...
    try {
      ...
      if (pendingFlagChanges.length > 0) {
        for (const change of pendingFlagChanges) {
          for (const c of conversations) {
            const affectedCount = (c.messageIds || []).filter((id: string) =>
              change.messageIds.includes(id)
            ).length;
            if (affectedCount > 0) {
  ...
  async function loadMoreSearchResults() {
    ...
    try {
      let results: any[] = [];
      if (isUnifiedView) {
        results = await SearchUnifiedInbox(
          query,
          newOffset,
          PAGE_SIZE,
          filterMode
  ...
  async function performServerSearch(limit: number = SERVER_SEARCH_LIMIT) {
    ...
    error = null;
    try {
      const response = await IMAPSearchFolder(
        accountId,
        folderId,
        query,
        limit
  ...
  function selectConversation(
    ...
    if (event?.shiftKey) {
      const start =
        lastClickedIndex !== null ? Math.min(lastClickedIndex, index) : index;
      const end =
        lastClickedIndex !== null ? Math.max(lastClickedIndex, index) : index;
    ...
    const realFolderId =
      (isUnifiedView || isSearchMode) && conversation.folderId
        ? conversation.folderId
        : folderId!;
    const realAccountId =
      (isUnifiedView || isSearchMode) && conversation.accountId
  ...
  async function fetchAndSelectServerResult(
    ...
    try {
      const msg = await FetchServerMessage(
        realAccountId,
        realFolderId,
        conversation._uid
      );
      if (msg) {
        ...
        const idx = serverSearchResults.findIndex(
          (r) => r._uid === conversation._uid
        );
        if (idx >= 0) {
  ...
  async function toggleSortOrder() {
    const newOrder =
      getMessageListSortOrder() === "newest" ? "oldest" : "newest";
    try {
  ...
  export function openSelected() {
    ...
    if (index >= 0) {
      ...
      const realFolderId =
        (isUnifiedView || isSearchMode) && conv.folderId
          ? conv.folderId
          : folderId!;
      const realAccountId =
        (isUnifiedView || isSearchMode) && conv.accountId
          ? conv.accountId
          : accountId!;
  ...
  export function getSelectedConversationInfo(): {
    accountId: string;
    folderId: string;
  } | null {
...
<div class="flex h-full flex-col {isFlashing ? 'pane-focus-flash' : ''}">
  ...
  >
    <div class="gap-2 flex items-center">
      ...
      {#if showSearch}
        <!-- Search input -->
        <div class="gap-1 bg-muted rounded-md px-2 flex flex-1 items-center">
          <Icon
            icon="mdi:magnify"
          ...
          <input
            ...
            class="text-sm py-1.5 w-full min-w-[200px] border-none bg-transparent outline-none"
          ...
          {#if serverSearchMode}
            <button
              ...
              class="px-1.5 py-0.5 font-medium bg-primary/20 text-primary hover:bg-primary/30 flex-shrink-0 rounded-full text-[10px] transition-colors"
    ...
    <div class="gap-1 flex items-center">
      {#if syncing}
        ...
        <button
          ...
          title={syncProgress
            ? `${$_("sidebar.syncing")} ${syncProgress.phase}: ${syncProgress.percentage}% - ${$_("sidebar.clickToCancel")}`
            : `${$_("sidebar.syncing")} ${$_("sidebar.clickToCancel")}`}
      ...
      {:else}
        ...
        <DropdownMenu.Root>
          ...
          >
            <Icon icon="mdi:refresh" class="w-5 h-5 text-muted-foreground" />
          </DropdownMenu.Trigger>
          <DropdownMenu.Portal>
            <DropdownMenu.Content
              ...
              sideOffset={4}
              class={cn(
                "rounded-md bg-popover p-1 text-popover-foreground shadow-md z-50 min-w-[180px] border",
            ...
            >
              <DropdownMenu.Item
                onSelect={syncFolder}
                class="rounded-sm px-2 py-1.5 text-sm focus:bg-accent focus:text-accent-foreground relative flex cursor-default items-center outline-none select-none"
              >
                <Icon icon="mdi:refresh" class="w-4 h-4 mr-2" />
                {$_("messageList.syncFolder")}
              </DropdownMenu.Item>
              <DropdownMenu.Separator class="-mx-1 my-1 bg-border h-px" />
              <DropdownMenu.Item
      ...
      {/if}
      <button
        class="p-2 rounded-md hover:bg-muted transition-colors {showSearch
          ? 'bg-muted'
          : ''}"
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger
          class="p-2 rounded-md hover:bg-muted transition-colors {filterMode
            ? 'bg-muted'
            : ''}"
          ...
          >
            {#each filterOptions as opt}
              ...
              >
                <Icon
                  icon="mdi:check"
                  class="w-4 h-4 mr-2 {filterMode === opt.value
                    ? ''
                    : 'invisible'}"
                />
                {opt.label}
      ...
      <button
        class="p-2 rounded-md hover:bg-muted transition-colors"
        title={getMessageListSortOrder() === "newest"
          ? $_("messageList.showingNewest")
          : $_("messageList.showingOldest")}
        onclick={toggleSortOrder}
      >
        <Icon
          icon={getMessageListSortOrder() === "newest"
            ? "mdi:sort-descending"
            : "mdi:sort-ascending"}
          class="w-5 h-5 text-muted-foreground"
        />
  ...
  {#if filterMode}
    <div
      class="gap-2 px-4 py-1.5 border-border bg-muted/30 flex items-center border-b"
    >
      <span class="text-xs text-muted-foreground"
        >{$_("messageList.filterLabel")}:</span
      >
      <button
        class="gap-1 px-2 py-0.5 text-xs bg-primary/10 text-primary hover:bg-primary/20 inline-flex items-center rounded-full transition-colors"
        onclick={() => setFilter("")}
      >
        {filterLabel}
  ...
  {#if folderType === "trash" && totalCount > 0 && !isSearchMode}
    <div
      class="px-4 py-2 bg-muted/50 border-border flex items-center justify-end border-b"
    >
      <Button
        size="sm"
        variant="outline"
  ...
  {#if showSearch && !indexComplete && isIndexing}
    <div class="px-4 py-2 bg-muted/50 border-border border-b">
      <div class="gap-2 text-sm text-muted-foreground flex items-center">
        <Icon icon="mdi:database-sync" class="w-4 h-4 animate-pulse" />
        <span
          >{$_("messageList.ftsBuilding", {
            values: { percentage: indexProgress }
          })}</span
        >
      </div>
      <div class="h-1 bg-muted mt-1.5 overflow-hidden rounded-full">
        <div
  ...
  >
    ...
    {:else if error}
      <div
        class="h-32 px-4 flex flex-col items-center justify-center text-center"
      >
        <Icon
          icon="mdi:alert-circle-outline"
          class="w-8 h-8 text-destructive mb-2"
        />
    ...
    {:else if isSearchMode}
      <!-- Search Results -->
      {#if isSearching || isServerSearching}
        <div class="h-32 gap-2 flex flex-col items-center justify-center">
          <Icon
            icon="mdi:loading"
            class="w-6 h-6 animate-spin text-muted-foreground"
          />
      ...
      {:else if serverSearchMode}
        ...
        {#if serverSearchResults.length === 0}
          ...
          >
            ...
            <p>
              {$_("messageList.noResults", { values: { query: searchQuery } })}
            </p>
          </div>
        {:else}
          <!-- Server results header -->
          <div
            class="px-4 py-2 bg-muted/30 border-border text-sm text-muted-foreground flex items-center justify-between border-b"
          >
            <span>
              {#if serverSearchCount < serverSearchTotalCount}
                {$_("search.serverResultsCapped", {
                  values: {
                    shown: serverSearchCount,
                    total: serverSearchTotalCount,
                    query: searchQuery
                  }
                })}
              {:else}
                {$_("search.serverResults", {
                  values: { count: serverSearchCount, query: searchQuery }
                })}
              {/if}
            </span>
            <button
          ...
          {#each serverSearchResults as result, index (result.threadId + "-" + index)}
            ...
            <ConversationRow
              ...
              onCheck={(checked, e) =>
                handleCheck(result.threadId, checked, index, e)}
          ...
          {#if serverSearchCount < serverSearchTotalCount}
            <div class="py-4 flex justify-center">
              <button
                ...
                class="text-sm text-primary focus:ring-primary rounded px-2 py-1 hover:underline focus:ring-2 focus:ring-offset-2 focus:outline-none"
                onclick={() => performServerSearch(0)}
                disabled={isServerSearching}
              >
                {isServerSearching
                  ? $_("common.loading")
                  : $_("search.showAllResults", {
                      values: { total: serverSearchTotalCount }
                    })}
              </button>
            </div>
          {/if}
        ...
        >
          ...
          {#if !isUnifiedView && accountId && folderId}
            <button
              ...
              onclick={() => {
                serverSearchMode = true;
                lastServerQuery = searchQuery.trim();
                performServerSearch();
              }}
            >
      ...
      {:else}
        ...
        >
          <span
            >{$_("messageList.foundResults", {
              values: { count: searchTotalCount, query: searchQuery }
            })}</span
          >
          {#if !isUnifiedView && accountId && folderId}
            <button
        ...
        {#if searchResults.length < searchTotalCount}
          <div class="py-4 flex justify-center">
            ...
            >
              {isSearching
                ? $_("common.loading")
                : $_("messageList.loadMore", {
                    values: {
                      remaining: searchTotalCount - searchResults.length
                    }
                  })}
            </button>
          </div>
    ...
    {:else}
      {#each conversations as conv, index (conv.threadId)}
        ...
        <ConversationRow
          ...
          onCheck={(checked, e) =>
            handleCheck(conv.threadId, checked, index, e)}
          onClearSelection={clearSelection}
          onActionComplete={handleActionComplete}
          {onReply}
        />
        ...
        <div class="py-4 flex justify-center">
          ...
          >
            {loading
              ? $_("common.loading")
              : $_("messageList.loadMore", {
                  values: { remaining: totalCount - conversations.length }
                })}
          </button>
        </div>
      {/if}
