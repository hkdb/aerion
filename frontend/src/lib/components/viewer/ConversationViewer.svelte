<script lang="ts">
  ...
  onMount(async () => {
    ...
    cleanupFunctions.push(
      EventsOn(
        "messages:flagsChanged",
        (data: { messageIds: string[]; isRead: boolean }) => {
          // Check if this is our own mark-as-read operation
          const isOwnOperation = data.messageIds.every((id) =>
            pendingMarkAsReadIds.has(id)
          );
          ...
          if (isOwnOperation) {
            ...
            if (conversation?.messages) {
              ...
              const delta = data.isRead
                ? -data.messageIds.length
                : data.messageIds.length;
              conversation.unreadCount = Math.max(
                0,
                (conversation.unreadCount || 0) + delta
              );
              // Trigger reactivity
    ...
    cleanupFunctions.push(
      EventsOn(
        "messages:moved",
        (data: { messageIds: string[]; destFolderId: string }) => {
          if (
            !conversation?.messages?.some((m) => data.messageIds.includes(m.id))
          )
            return;

          const movedCount = conversation.messages.filter((m) =>
            data.messageIds.includes(m.id)
          ).length;
    ...
    cleanupFunctions.push(
      EventsOn("messages:deleted", async (messageIds: string[]) => {
        if (conversation?.messages?.some((m) => messageIds.includes(m.id))) {
          ...
          const deletedCount = conversation.messages.filter((m) =>
    ...
    cleanupFunctions.push(
      EventsOn(
        "messages:updated",
        (data: { accountId: string; folderId: string }) => {
          if (
            threadId &&
            folderId &&
            accountId &&
  ...
  function scheduleMarkAsRead(
    ...
    messages: messageModels.Message[]
  ) {
  ...
  export function selectAllText() {
    ...
    if (!messageEl) return;
    const iframe = messageEl.querySelector(
      "iframe"
...
<div class="flex h-full flex-col {isFlashing ? 'pane-focus-flash' : ''}">
  {#if !threadId}
  ...
  {:else if error}
    <!-- Error -->
    <div
      class="px-4 flex h-full flex-col items-center justify-center text-center"
    >
      <Icon
        icon="mdi:alert-circle-outline"
        class="w-12 h-12 text-destructive mb-3"
      />
  ...
  {:else if conversation}
    <div class="conversation-viewer-content flex h-full flex-col">
      <!-- Header with Actions -->
      <div
      ...
      >
        <div class="gap-2 flex items-center">
          {#if showBackButton}
            <button
              class="p-2 rounded-md hover:bg-muted mr-1 transition-colors"
            ...
            <div class="h-5 bg-border mx-1 w-px"></div>
          {/if}
          <button
          ...
          <button
            class="p-2 rounded-md hover:bg-muted transition-colors"
            title={$_(
              isTrashFolder ? "viewer.deletePermanently" : "viewer.delete"
            )}
          ...
          <button
            class="p-2 rounded-md hover:bg-muted transition-colors"
            title={$_(
              isSpamFolder ? "viewer.markAsNotSpam" : "viewer.markAsSpam"
            )}
            onclick={handleSpam}
          >
            <Icon
          ...
          >
            <Icon
              icon={allStarred ? "mdi:star" : "mdi:star-outline"}
              class="w-5 h-5 {allStarred
                ? 'text-yellow-500'
          ...
          >
            <Icon
              icon={allRead ? "mdi:email-open-outline" : "mdi:email-outline"}
              class="w-5 h-5 text-muted-foreground"
            />
          </button>
        </div>
      ...
      <div
        ...
        class="min-h-0 scrollbar-thin flex-1 overflow-y-auto"
        ...
        <div class="p-6">
          ...
          {#if conversation.messages && conversation.messages.length > 1}
            <div class="mb-4 text-sm text-muted-foreground">
              {$_("viewer.messagesInConversation", {
                values: { count: conversation.messages.length }
              })}
            </div>
          {/if}
          ...
          {#if conversation.messages}
            <div class="space-y-4">
              {#each conversation.messages as msg, index (msg.id)}
                ...
                >
                  <div
                    class="rounded-lg overflow-hidden border transition-all {focusedMessageId ===
                    msg.id
                    ...
                    onblur={() => {
                      if (focusedMessageId === msg.id) focusedMessageId = null;
                    }}
                  >
                    <!-- Message Header (always visible, clickable to expand/collapse) -->
                    <!-- svelte-ignore a11y_no_static_element_interactions -->
                    <div
                      class="gap-3 p-4 hover:bg-muted/50 flex w-full cursor-pointer items-start text-left transition-colors {!isExpanded
                        ? 'bg-muted/30'
                        : ''}"
                      ...
                      <div class="min-w-0 flex-1">
                        <div class="gap-2 flex flex-wrap items-center">
                          <span class="font-medium text-foreground"
                            >{msg.fromName || $_("viewer.unknown")}</span
                          >
                          <span
                            role="button"
                            tabindex="0"
                            class="text-sm text-muted-foreground hover:text-primary cursor-pointer hover:underline"
                            title={$_("viewer.copyEmail")}
                            onclick={(e) => {
                              e.stopPropagation();
                              copyToClipboard(
                                formatEmailForCopy(msg.fromName, msg.fromEmail),
                                $_("viewer.from")
                              );
                            }}
                            onkeydown={(e) => {
                          ...
                          {#if !msg.isRead}
                            <span
                              class="w-2 h-2 bg-primary flex-shrink-0 rounded-full"
                            ></span>
                          {/if}
                        </div>
                        ...
                        {#if msg.replyTo && msg.replyTo.toLowerCase() !== msg.fromEmail.toLowerCase()}
                          <div
                            class="text-sm text-muted-foreground gap-1 flex flex-wrap items-center"
                          >
                            <span class="opacity-60"
                              >{$_("viewer.replyTo")}</span
                            >
                            <span
                              role="button"
                              tabindex="0"
                              class="hover:text-primary text-muted-foreground cursor-pointer hover:underline"
                              title={$_("viewer.copyEmail")}
                              onclick={(e) => {
                                e.stopPropagation();
                                copyToClipboard(
                          ...
                          >
                            ...
                            {#each recipients as recipient, i}
                              <span
                                ...
                                >{recipient.name || recipient.email}{i <
                                recipients.length - 1
                                  ? ","
                                  : ""}</span
                              >
                    ...
                    {#if isExpanded}
                      <div class="px-4 pb-4 pt-0">
                        <div class="ml-13 pl-3 border-border border-l-2">
                          <!-- Read Receipt Banner -->
                          {#if shouldShowReadReceiptBanner(msg) && readReceiptPolicy === "ask"}
                            <div
                              class="gap-3 px-3 py-2 mb-4 bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800 rounded-md flex items-center justify-between border"
                            >
                              <div
                                class="gap-2 text-sm text-blue-700 dark:text-blue-300 flex items-center"
                              >
                                <Icon
                          ...
                          {:else if shouldShowReadReceiptBanner(msg) && readReceiptPolicy === "always" && sendingReadReceipt.has(msg.id)}
                            <div
                              class="gap-2 px-3 py-2 mb-4 bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800 rounded-md text-sm text-green-700 dark:text-green-300 flex items-center border"
                            >
                              <Icon
                                icon="mdi:loading"
                          ...
                          {#if smimeLoading.has(msg.id)}
                            <div
                              class="gap-2 px-3 py-2 mb-4 bg-muted/50 border-border rounded-md text-sm text-muted-foreground flex items-center border"
                            >
                              <Icon
                                icon="mdi:loading"
                          ...
                          {#if smimeResults[msg.id]?.smimeEncrypted}
                            <div
                              class="gap-2 px-3 py-2 mb-4 bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800 rounded-md text-sm text-blue-700 dark:text-blue-300 flex items-center border"
                            >
                              <Icon
                                icon="mdi:lock-check"
                                class="w-4 h-4 flex-shrink-0"
                          ...
                          {#if (msg.hasSMIME ? smimeResults[msg.id]?.smimeStatus : msg.smimeStatus) === "signed"}
                            ...
                            >
                              ...
                              />
                              <span
                                >{$_("viewer.smimeSignedBy", {
                                  values: {
                                    email:
                                      (msg.hasSMIME
                                        ? smimeResults[msg.id]?.smimeSignerEmail
                                        : msg.smimeSignerEmail) ||
                                      (msg.hasSMIME
                                        ? smimeResults[msg.id]
                                            ?.smimeSignerSubject
                                        : msg.smimeSignerSubject) ||
                                      $_("viewer.unknown").toLowerCase()
                                  }
                                })}</span
                              >
                            </div>
                          {:else if (msg.hasSMIME ? smimeResults[msg.id]?.smimeStatus : msg.smimeStatus) === "unknown_signer"}
                            <div
                              class="gap-2 px-3 py-2 mb-4 bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800 rounded-md text-sm text-amber-700 dark:text-amber-300 flex items-center border"
                            >
                              <Icon
                          ...
                          {:else if (msg.hasSMIME ? smimeResults[msg.id]?.smimeStatus : msg.smimeStatus) === "expired_cert"}
                            <div
                              class="gap-2 px-3 py-2 mb-4 bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800 rounded-md text-sm text-red-700 dark:text-red-300 flex items-center border"
                            >
                              <Icon
                                icon="mdi:shield-off"
                                class="w-4 h-4 flex-shrink-0"
                          ...
                          {#if pgpResults[msg.id]?.pgpStatus === "signed"}
                            ...
                            >
                              ...
                              />
                              <span
                                >{$_("viewer.pgpSignedBy", {
                                  values: {
                                    email:
                                      pgpResults[msg.id]?.pgpSignerEmail ||
                                      $_("viewer.unknown").toLowerCase()
                                  }
                                })}</span
                              >
                          ...
                          {:else if pgpResults[msg.id]?.pgpStatus === "unknown_key"}
                            ...
                            >
                              ...
                              />
                              <span
                                >{$_("viewer.pgpUnknownKey", {
                                  values: {
                                    keyId:
                                      pgpResults[msg.id]?.pgpSignerKeyId || ""
                                  }
                                })}</span
                              >
                            </div>
                          ...
                          <div class="mb-4">
                            {#if (msg as any).bodyFetched === false && !msg.bodyHtml && !msg.bodyText}
                              ...
                              <div
                                class="gap-2 text-muted-foreground text-sm py-4 flex items-center italic"
                              >
                                <Icon
                                  icon="mdi:loading"
                                  class="w-4 h-4 animate-spin"
                            ...
                            {:else if (msg.hasSMIME && !smimeResults[msg.id] && smimeLoading.has(msg.id)) || (msg.hasPGP && !pgpResults[msg.id] && pgpLoading.has(msg.id))}
                              ...
                              <div
                                class="text-muted-foreground text-sm py-4 italic"
                              >
                                {$_("viewer.decryptingMessage")}
                              </div>
                            {:else}
                              <EmailBody
                                messageId={msg.id}
                                accountId={msg.accountId}
                                bodyHtml={msg.hasPGP && pgpResults[msg.id]
                                  ? pgpResults[msg.id].bodyHtml
                                  : msg.hasSMIME && smimeResults[msg.id]
                                    ? smimeResults[msg.id].bodyHtml
                                    : msg.bodyHtml}
                                bodyText={msg.hasPGP && pgpResults[msg.id]
                                  ? pgpResults[msg.id].bodyText
                                  : msg.hasSMIME && smimeResults[msg.id]
                                    ? smimeResults[msg.id].bodyText
                                    : msg.bodyText}
                                fromEmail={msg.fromEmail}
                                onCompose={onComposeToAddress}
                                onImagesLoaded={() =>
                                  messagesWithImagesLoaded.add(msg.id)}
                                encryptedInlineAttachments={pgpResults[msg.id]
                                  ?.inlineAttachments ??
                                  smimeResults[msg.id]?.inlineAttachments}
                              />
                            {/if}
                          </div>
                          ...
                          {#if msg.hasAttachments || (pgpResults[msg.id]?.attachments?.length ?? 0) > 0 || (smimeResults[msg.id]?.attachments?.length ?? 0) > 0}
                            <div class="border-border pt-4 mt-4 border-t">
                              <h3
                                class="text-sm font-medium text-foreground mb-3 gap-2 flex items-center"
                              >
                              ...
                              <AttachmentList
                                messageId={msg.id}
                                encryptedAttachments={pgpResults[msg.id]
                                  ?.attachments ??
                                  smimeResults[msg.id]?.attachments}
                              />
                            </div>
                          {/if}
                          ...
                          <div class="border-border pt-4 mt-4 border-t">
                            <button
                              ...
                              class="gap-2 text-sm text-muted-foreground hover:text-foreground flex items-center transition-colors"
                            >
                              <Icon
                                icon={viewingSourceMessageId === msg.id
                                  ? "mdi:code-tags"
                                  : "mdi:code-tags"}
                                class="w-4 h-4"
                              />
                              {viewingSourceMessageId === msg.id
                                ? $_("viewer.hideSource")
                                : $_("viewer.viewSource")}
                            </button>
                            ...
                            {#if viewingSourceMessageId === msg.id}
                              <div class="mt-3">
                                {#if loadingSource}
                                  <div
                                    class="gap-2 text-sm text-muted-foreground flex items-center"
                                  >
                                    <Icon
                                      icon="mdi:loading"
                                      class="w-4 h-4 animate-spin"
                                ...
                                {:else if messageSource}
                                  <div class="relative">
                                    <button
                                      ...
                                      class="top-2 right-2 p-1.5 rounded bg-muted hover:bg-muted/80 absolute transition-colors"
                                    ...
                                    <pre
                                      class="text-xs bg-muted/50 p-4 rounded-md max-h-96 font-mono overflow-x-auto overflow-y-auto break-all whitespace-pre-wrap">{messageSource}</pre>
                                  </div>
                                {/if}
                              </div>
                            {/if}
