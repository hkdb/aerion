<script lang="ts">
  ...
  // @ts-ignore - wailsjs path
  import { certificate, folder, smtp } from "../wailsjs/go/models";
  ...
  async function handleBgCertAcceptOnce() {
    ...
    try {
      ...
      const acc = accountStore.accounts.find(
        (a) => a.account.id === pendingCertAccountId
      );
  ...
  function findFolderById(
    accountId: string,
    folderId: string
  ): { name: string; type: string; path: string } | null {
  ...
  onMount(async () => {
    ...
    EventsOn(
      ...
      (data: { accountId: string; folderId: string; threadId: string }) => {
    ...
    try {
      ...
      if (
        mailtoData &&
        (mailtoData.to?.length > 0 || mailtoData.subject || mailtoData.body)
      ) {
  ...
  async function handleEditDraft(draftId: string) {
    ...
    const accountId =
      resolveAccountId(selectedConversationAccountId) ||
      resolveAccountId(selectedAccountId);
    if (!accountId) return;
  ...
  function handleMailtoData(data: MailtoData, rawMailtoURL?: string) {
    ...
    composerInitialMessage = new smtp.ComposeMessage({
      from: new smtp.Address({ name: "", address: "" }),
      to: (data.to || []).map(
        (addr) => new smtp.Address({ name: "", address: addr })
      ),
      cc: (data.cc || []).map(
        (addr) => new smtp.Address({ name: "", address: addr })
      ),
  ...
  function handleMouseMove(e: MouseEvent) {
    if (isResizingSidebar) {
      sidebarWidth = Math.max(
        paneConstraints.sidebar.min,
        Math.min(paneConstraints.sidebar.max, e.clientX)
      );
    } else if (isResizingList) {
      listWidth = Math.max(
        paneConstraints.list.min,
        Math.min(paneConstraints.list.max, e.clientX - sidebarWidth)
      );
    }
  }
    ...
    if (e.ctrlKey || e.metaKey) {
      switch (e.key.toLowerCase()) {
        ...
        case "r": {
          ...
          if (!msgId) return;
          handleReply(
            e.shiftKey ? "reply-all" : "reply",
            msgId,
            viewerRef?.isImagesLoaded(msgId) || false
          );
          return;
        }
        case "f": {
    ...
    switch (e.key) {
      ...
      case "Enter":
        ...
        if (document.activeElement?.tagName === "BUTTON") {
          ...
          if (
            (focusedPane === "messageList" && inMessageList) ||
        ...
        if (focusedPane === "sidebar" && sidebarRef?.hasFocusedAccount()) {
          ...
          focusedPane === "sidebar" &&
          sidebarRef?.hasSelectedFolderWithChildren()
        ) {
          e.preventDefault();
          sidebarRef.toggleSelectedFolderCollapse();
        } else if (focusedPane === "messageList") {
  ...
  async function handleBulkArchive(messageIds: string[]) {
    try {
      ...
      addToast({
        type: "success",
        message: $_("toast.archived"),
        actions: [{ label: $_("common.undo"), onClick: handleUndo }]
      });
      messageListRef?.clearChecked();
  ...
  async function handleBulkSpam(messageIds: string[]) {
    try {
      ...
      if (isSpamFolder) {
        ...
        addToast({
          type: "success",
          message: $_("toast.markedAsNotSpam"),
          actions: [{ label: $_("common.undo"), onClick: handleUndo }]
        });
      } else {
...
<div class="bg-background flex h-full w-full flex-col overflow-hidden">
  ...
  <!-- Main Content -->
  <div class="min-h-0 relative flex flex-1 overflow-hidden">
    <!-- Sidebar (Folder List) -->
    <aside
      class={getLayoutMode() === "narrow"
        ? `responsive-sidebar-overlay w-72 border-border bg-background border-r ${getResponsiveView() === "sidebar" ? "responsive-sidebar-visible" : ""}`
        : "border-border bg-muted/30 flex-shrink-0 border-r"}
    ...
    {#if getLayoutMode() === "narrow"}
      ...
      <div
        role="button"
        tabindex="-1"
        class="responsive-scrim {getResponsiveView() === 'sidebar'
          ? 'responsive-scrim-visible'
          : ''}"
        onclick={hideSidebar}
    ...
    {#if getLayoutMode() === "full"}
      <button
        type="button"
        class="w-1 hover:bg-primary/20 active:bg-primary/40 p-0 cursor-col-resize border-0 transition-colors {isResizingSidebar
          ? 'bg-primary/40'
          : ''}"
    ...
    <section
      ...
      class={isResponsive()
        ? "min-w-0 border-border bg-background flex-1 border-r"
        : "border-border bg-background flex-shrink-0 border-r"}
    ...
    {#if getLayoutMode() === "full"}
      <button
        type="button"
        class="w-1 hover:bg-primary/20 active:bg-primary/40 p-0 cursor-col-resize border-0 transition-colors {isResizingList
          ? 'bg-primary/40'
          : ''}"
    ...
    <main
      class={isResponsive()
        ? `responsive-viewer-overlay bg-background ${getResponsiveView() === "viewer" ? "responsive-viewer-visible" : ""}`
        : "min-w-0 bg-background flex-1"}
      role="presentation"
      data-pane="viewer"
    ...
    >
      <ConversationViewer
        ...
        onActionComplete={(autoSelectNext) =>
...
{#if isResizingSidebar || isResizingList}
  <div class="inset-0 fixed z-50 cursor-col-resize"></div>
{/if}
...
{#if showComposer && composerAccountId}
  <div class="inset-0 bg-black/50 fixed z-50 flex items-center justify-center">
    <div
      class={getLayoutMode() === "narrow"
        ? "bg-background h-full w-full overflow-hidden"
        : "max-w-3xl bg-background rounded-lg shadow-xl h-[80vh] w-full overflow-hidden"}
    >
      <Composer
...
{#if isShuttingDown}
  <div
    class="inset-0 bg-black/80 fixed z-[100] flex items-center justify-center"
  >
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        ...
        <pre class="mb-3 rounded bg-muted p-2 text-sm overflow-x-auto"><code
            >flatpak override --user --filesystem=home com.aerion.Aerion</code
          ></pre>
        <p class="mb-3 text-sm text-destructive">
          {$_("attachment.flatpakOpenSecurityWarning")}
        </p>
        <p class="text-sm text-muted-foreground">
          {$_("attachment.flatpakOpenAlternative")}
        </p>
      </AlertDialog.Description>
    </AlertDialog.Header>
    <AlertDialog.Footer>
      <AlertDialog.Action onclick={() => (showFlatpakFsDialog = false)}
